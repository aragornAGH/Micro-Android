 ...
 */
public class ExecutionTimeInstanceManager extends AbstractKnowledgeInstanceManager<LearningParameters> {
    @Override
    protected Instances buildNewWekaData() {
        ...
        Attribute executionTime = new Attribute(WekaConstants.EXECUTION_TIME);
        ...
        wekaAttributes.add(executionTime);

        Instances trainingSet = new Instances("conversion", wekaAttributes, 0);
        trainingSet.setClass(executionTime);
        return trainingSet;
    }
    ...
    public void addNewInstance(LearningParameters parameters) {
        ...
        newInstance.setValue(data.attribute(6), parameters.getExecutionTime());
